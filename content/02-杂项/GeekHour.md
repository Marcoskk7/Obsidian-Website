...
需要注意的是, 同一个消息只能被一个消费者组的一个消费者消费, 但是可以被不同消费者组进行消费, 这样保证了一条消息可以在不同业务系统中处理, 比如一个订单, 可以同时扣减库存, 扣减余额, 增加积分等

Kafka 跑在 jvm 上, 数据还得写磁盘, 两个听着慢的操作, 为什么使用起来快?
- 磁盘的顺序访问速度远远快于随机访问的速度 , 而 Kafka 只追加不修改, 把磁盘随机写改为顺序写, 吃满了顺序写磁盘的速度红利![image.png](https://img.486597.xyz/img/20260908183906633.png)
- Kafka 用批量刷盘换性能(积攒数据再写入磁盘), 单点丢数据不怕, 有副本机制兜底
- Kafka 是 IO 密集型应用, 零拷贝有效降低了其中的无意义性能损耗